...
function Services() {
  const {locale} = useContext(langContext);
	...
	useEffect(() => {
		document.title = 'OXYHACK - Services';
	}, []);
    ...
    <>
      ...
      <main className='min-h-screen py-20 bg-background font-forma md:py-40'>
        ...
        <section className='max-w-7xl mx-auto flex flex-col justify-center items-center gap-3 px-4 text-center md:px-16'>
          <h1 className='w-4/5 max-w-5xl mx-auto text-3xl font-bold bg-gradient-to-l from-[#950101] via-[#BC2D2D] to-[#EC6464] text-transparent bg-clip-text md:text-6xl'>{locale.services_heroParagraph}</h1>
          <p className='text-lg font-vietnam font-normal text-dimmed md:text-2xl md:max-w-[50ch]'>{locale.services_subHero}</p>
          <CTAButton title={locale.services_CTAButton} href="#" />
          {/* Dummy hero image */}
        ...
        <section className='max-w-7xl flex flex-col gap-6 mx-4 mt-12 md:mx-16 xl:mx-auto'>
          {
            locale.drawers.map((drawer, i) => (
              <MouseHighlighter key={i}>
                <Drawer header={drawer.header} content={drawer.content} />
              </MouseHighlighter>
            ))
          }
        </section>
        ...
        <section className='max-w-7xl mx-auto mt-16 px-4 md:px-16 md:mt-36'>
          ...
          <div className='px-3 py-6 rounded-3xl bg-background-accent md:flex md:gap-10 md:justify-between md:items-center md:p-14'>
            <h4 className='mb-6 text-2xl text-primary font-medium font-grotesk md:mb-0'>{locale.section2_h4}</h4>
            <CTAButton href="#" title={locale.CTAButton} />
          </div>
          </MouseHighlighter>
        </section>
